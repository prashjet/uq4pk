...
def blob_plot(image: np.ndarray, blobs, savename: str, resolution: float = None,
    ...
    for blob in blobs:
        y, x, area = blob
        ax.add_patch(plt.Circle((x, y), np.sqrt(2) * area, color=scalecol,
                                fill=False))
        # If `resolution` is not None, add a second circle of size ``scale - resolution``
        if resolution is not None:
            min_area = area - resolution + 0.1
