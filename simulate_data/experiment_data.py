...
class ExperimentData:
    ...
    :ivar y: The noisy measurement.
    :ivar y_sd: The vector of standard deviations of the measurement noise.
    ...
    def get_forward_operator(self) -> ForwardOperator:
        """
        Creates the model forward operator.
        """
        op = ForwardOperator(hermite_order=self.hermite_order, mask=self.mask, ssps=self.ssps)
        return op


def save_experiment_data(data: ExperimentData, savename: str):
    """
...
def load_experiment_data(savedir: str) -> ExperimentData:
    ...
    data = ExperimentData(name=info_array[0], snr=snr, hermite_order=hermite_order, grid_type=info_array[1],
