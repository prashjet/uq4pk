...
from uq4pk_fit.regop import OrnsteinUhlenbeck, DiagonalOperator


class PixelModel(Model):
    """
    Implements a basic pixel-by-pixel model with standard l2-regularization:
    ...
    def __init__(self, y, standard_deviation, logname="pixel_model"):
        ...
        print(f"Pixel-based model successfully initialized.")
    ...
    def _assemble_fitted_model(self, xmap, precision, costfun, info):
        fitted_model = FittedPixelModel(model=self, xmap=xmap, precision=precision, costfun=costfun,
                                        info=info)
        return fitted_model


class FittedPixelModel(FittedModel):
    """
    Represents the fitted model, i.e. the PixelModel linearized around the map:
        ...
        f >= 0.
    """
    def __init__(self, model: PixelModel, xmap, precision, costfun, info):
