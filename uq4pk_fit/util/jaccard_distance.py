...
def mean_jaccard_distance(interval1: np.ndarray, interval2: np.ndarray) -> float:
    ...
    Parameters
    ----------
    interval1 : shape (dim, 2)
        The first column corresponds to the lower bound `a` and the second column corresponds to the upper bound `b`.
    interval2 : shape (dim, 2)
        The first column corresponds to the lower bound `c` and the second column corresponds to the upper bound `d`.

    Returns
    -------
    bar_d_j
        A number between 0 and 1, where 0 corresponds to identity and 1 corresponds to disjointness.
    """
