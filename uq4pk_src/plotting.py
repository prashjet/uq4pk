...
class Plotter:
    ...
    def plot_ssps(self, y_offset=0, lmd_rng=None):
        ...
        v = [np.array([self.ssps.lmd, models[i]+i*y_offset]).T for i in range(self.ssps.p)]
        cols = np.linspace(0, 1, self.ssps.p)
        if self.ssps.npars==1:
    ...
    def plot_df_mrg_1d(self,
                       ax=None,
                       axis=0,
                       bcis=[0.80, 0.95, 0.99]):
        if ax is None:
            ax = plt.gca()
        assert axis < self.ssps.npars
        x = self.ssps.reshape_beta(self.beta_smp.x)
        sum_axes = np.arange(1, self.ssps.npars+1)
        sum_axes = sum_axes[sum_axes != axis+1]
        sum_axes = tuple(sum_axes)
        x = np.sum(x, sum_axes)
        x = Samples(x)

        mode = x.get_halfsample_mode()
        # xarr = self.ssps.par_cents[axis]
        xarr = np.arange(self.ssps.par_dims[axis])

        ax.plot(xarr, mode, '-', color='C0')
        hdis = x.highest_density_bcis(p_in=bcis)
        for i in range(len(bcis)):
            ax.fill_between(xarr,
                            hdis[i,0], hdis[i,1],
                            color='C0', alpha=0.2)
