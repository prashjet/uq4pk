...
class Samples:
    ...
    def __init__(self, x=None, logprob=None, extras=None):
        if x.ndim == 1:
            x = x[np.newaxis, :]
        self.x = x
    ...
    def get_betafit_mode(self):
        ...
        arr mode_betafit, with shape = smp_shape
            contains NaN where distribution is estaimted to be flat/multi-modal
    ...
    @staticmethod
    def highest_density_single_bci(sx,      # sorted samples
                                   p_in,    # fraction inside desired BCI
                                   return_idx_and_n=False): # return option
        """
    ...
    def gaussian_significances_1d(self,
        ...
        def my_halfnorm_cdf(ncnt, ntot, nmin):
            ...
            # i.e. normalised such that maximum significance
            # = stats.halfnorm.ppf(1-resid_nsmp_min/nsmp_total)
